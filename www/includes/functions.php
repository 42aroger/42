...
function show_folder($path,$b)
{
    ...
    foreach ($files as $value)
    {
        ...
        elseif (is_file($absolutePath))
        {
			...
			if (detectme($namefile) == "NFO")
				{ 
					echo '<td type="button" class="btn btn-primary btn-xs"><a href="#myModal" class="btn" id="openBtn">NFO</a></td>'; 
					$nfotrack = ''.$absolutePath.'';
				}
			else 
				{ echo '<td class="categorie '.detectme($namefile).'"></td>'; }
			echo '<td>';
			...
			echo '<a class="site" href="download.php?sfile='.$absolutePath.'">'.$name.'</a></td>';
			echo '<td>';
						...
						<div class="modal-dialog">
						  <div class="modal-content wide-modal">
							...
							<div class="modal-body">
							   <iframe src="readnfo.php?file='.$nfotrack.'" style="zoom:0.60" frameborder="0" height="400" width="950"></iframe>
							</div>
						  </div>
						</div>
					  </div>';
...
function detectme($namefile)
{
	$cat = "cat-divers";
		...
		//FICHIER NFO
		if(preg_match("#.nfo#i", $namefile)){$cat = "NFO";}
